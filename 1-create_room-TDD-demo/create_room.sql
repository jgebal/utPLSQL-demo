...
-- Create the missing table as specified in the test.
--  Design decisions
--  1. Use auto-generated identity for room-key
--  2. Make room_key the primary key for rooms table
--  3. Limit the room name to 100 characters
create table rooms (
...
pause
--------------------------------------------------------------
-- Run test suite - test is failing as the CREATE_ROOM function is not inserting any data.
-- Function is only a skeleton so far.
exec ut.run('test_create_room');
pause
clear screen
...
pause
--------------------------------------------------------------
-- Validate that the test is now passing
exec ut.run('test_create_room');
pause
clear screen
...
--
-- Function - Create room
--  Throws an exception when room name is longer than 100 characters

pause
-- Add name too long test
create or replace package test_create_room is
  --%suite(Function create_room)
  ...
  --%test(Throws exception when room name is longer than 100 characters)
  --%throws(-6502)
  procedure fail_room_name_too_long;
...
create or replace package body test_create_room is
  ...
  procedure fail_room_name_too_long is
    l_room_id integer;
  begin
    -- ACT
    l_room_id := create_room( lpad('Room name too long','-',120) );
    --ASSERT done via --%throws annotation in spec
  end;
...
pause
--------------------------------------------------------------
exec ut.run( 'test_create_room', a_color_console => true );
pause
clear screen
